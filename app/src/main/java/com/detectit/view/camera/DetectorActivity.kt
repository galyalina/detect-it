...
package com.detectit.view.camera
...
import com.detectit.detection.env.BorderedText
import com.detectit.detection.env.ImageUtils
import com.detectit.detection.env.Logger
import com.detectit.detection.general.MasterApplication
import com.detectit.model.location.PointOfInterestEntity
import com.detectit.detection.tflite.Classifier
import com.detectit.detection.tflite.TFLiteObjectDetectionAPIModel
import com.detectit.detection.tracking.MultiBoxTracker
import com.detectit.view.locations.MapsActivity
...
class DetectorActivity : CameraActivity(), OnImageAvailableListener {
    ...
    private var saveItemOnMap: Snackbar? = null
    ...
    private fun showSnackBar(title: String) {
        saveItemOnMap = Snackbar.make(toolbar, "Save detected item on map", Snackbar.LENGTH_INDEFINITE)
                .setAction("Save") {
                    (application as? MasterApplication)
                            ...
                            ?.subscribe(
                                    { location ->
                                        val point = PointOfInterestEntity().apply {
                                            this.setLatitude(location.latitude)
                                            this.setLongitude(location.longitude)
                                            this.setDescription("Image with detection")
                                            this.setName(title)
                                        }
                                        (application as? MasterApplication)
                                                ?.data
                                    ...
                                    {
                                        LOGGER.e("Failed to get location, ${it.localizedMessage}")
                                    }
                            )
                }
        saveItemOnMap?.show()
